#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────
# Arka Installer — Detects hardware, configures, and launches
#
# Usage:
...
ARKA_DIR="${ARKA_DIR:-$HOME/arka}"
ARKA_BRANCH="arka-build-alpha"
ARKA_REPO="https://github.com/shrijitb/MARA.git"

echo "═══════════════════════════════════════════════════"
echo "  ARKA — Agentic Risk-Kinetic Allocator            "
...
step "Setting up Arka in $ARKA_DIR"

mkdir -p "$ARKA_DIR"

if [ -d "$ARKA_DIR/.git" ]; then
    ok "Existing installation found. Pulling latest from $ARKA_BRANCH..."
    cd "$ARKA_DIR"
    git fetch origin
    git checkout "$ARKA_BRANCH" 2>/dev/null || true
    git pull origin "$ARKA_BRANCH"
else
    git clone -b "$ARKA_BRANCH" "$ARKA_REPO" "$ARKA_DIR"
    cd "$ARKA_DIR"
    ok "Cloned from $ARKA_REPO"
fi
...
detect_hardware() {
    ...
    cat > "$ARKA_DIR/.hardware_profile.json" << HWEOF
{
    "arch":         "$arch_label",
    "os":           "$os_id",
...
if command -v jq &>/dev/null; then
    DETECTED_RAM=$(jq -r '.ram_mb'  "$ARKA_DIR/.hardware_profile.json")
    DETECTED_BOARD=$(jq -r '.board' "$ARKA_DIR/.hardware_profile.json")
    DETECTED_ARCH=$(jq -r '.arch'   "$ARKA_DIR/.hardware_profile.json")
else
    DETECTED_RAM=$(awk -F'"' '/"ram_mb"/{print $0}' "$ARKA_DIR/.hardware_profile.json" | grep -o '[0-9]*' | head -1)
    DETECTED_BOARD=$(awk -F'"' '/"board"/{print $4}' "$ARKA_DIR/.hardware_profile.json")
    DETECTED_ARCH=$(awk -F'"' '/"arch"/{print $4}' "$ARKA_DIR/.hardware_profile.json")
fi
...
if [ ! -f "$ARKA_DIR/.env" ]; then
    cat > "$ARKA_DIR/.env" << ENVEOF
# ── Arka Configuration (auto-generated by installer) ──
...
cat > "$ARKA_DIR/docker-compose.profile.yml" << DCEOF
...
step "Building and starting Arka"
...
step "Waiting for Arka to become healthy"
